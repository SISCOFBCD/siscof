...
// //include("mostra-alerta.php");
	...
	if(!function_exists("buscarAlertasAbertos")) {

		function buscarAlertasAbertos($tipo, $Aid, $id_user){
			
			$tabela = retornaTabelaAlerta($tipo);
			$tipo_Aid = retornaIDAlerta($tipo);
			
			
			$query="select {$tipo_Aid} from {$tabela} where UPid = {$id_user}";
			$busca = db_mysql($query);
			
			$Aid_alertas = [];        
			while ($Aid_alerta = mysqli_fetch_array($busca)) {            
				$Aid_alertas[] = $Aid_alerta;
			}
			
			foreach ($Aid_alertas as $Aid) :
			
			if(seAnoFim_Igual($tipo,$Aid)) {
				if(seMesFim_Igual($tipo,$Aid)) {
					if(seDiaFim_Igual($tipo,$Aid)) {
						//return true;
						$query="select * from {$tabela} where DAY(dataFim) = DAY(NOW()) and MONTH(dataFim) = MONTH(NOW()) and YEAR(dataFim) = YEAR(NOW()) and UPid = {$id_user}";
					} elseif(seDiaFim_Maior($tipo,$Aid)) {
						//return true;
						$query="select * from {$tabela} where DAY(dataFim) > DAY(NOW()) and MONTH(dataFim) = MONTH(NOW()) and YEAR(dataFim) = YEAR(NOW()) and UPid = {$id_user}";
					} else {
						//return false;
				...
				} elseif(seMesFim_Maior($tipo,$Aid)) {
					//return true;
					$query="select * from {$tabela} where MONTH(dataFim) > MONTH(NOW()) and YEAR(dataFim) = YEAR(NOW()) and UPid = {$id_user}";
				} else {
					//return false;
					return null;
				}
					
			} elseif(seAnoFim_Maior($tipo,$Aid)) {
				//return true;
				$query="select * from {$tabela} where YEAR(dataFim) > YEAR(NOW()) and UPid = {$id_user}";
			} else
				//return false;
				return null;
	...
	if(!function_exists("buscarAlertasHConcluidos")) {
		...
		function buscarAlertasHConcluidos($id_user){
			...
			$query="select * from Alertas_Horarios where YEAR(dataFim) < YEAR(NOW()) and UPid = {$id_user}";
			$busca = db_mysql($query);
			
			$alertas1 = [];        
			while ($alerta1 = mysqli_fetch_array($busca)) {            
				$alertas1[] = $alerta1;
				if($alerta1 == null) $AH_finalizado = 1;	
			}
			
			
			// Verifica os campos do mês
			if($AH_finalizado>0) {
				$query="select * from Alertas_Horarios where MONTH(dataFim) < MONTH(NOW()) and UPid = {$id_user}";
				$busca = db_mysql($query);
				
				$alertas2 = [];        
				while ($alerta2 = mysqli_fetch_array($busca)) {            
					$alertas2[] = $alerta2;
					if($alerta2 == null) $AH_finalizado = 2;
				}
			}
			
			
			// Verifica os campos do dia
			if($AH_finalizado>2) {
				$query="select * from Alertas_Horarios where DAY(dataFim) < DAY(NOW()) and UPid = {$id_user}";
				$busca = db_mysql($query);
				
				$alertas3 = [];        
				while ($alerta3 = mysqli_fetch_array($busca)) {            
					$alertas3[] = $alerta3;
					if($alerta3 == null) $AH_finalizado = 3;						
				}
			}
			...
			if($AH_finalizado == 1) return $alertas1;
			elseif($AH_finalizado == 2) return $alertas2;
			elseif($AH_finalizado == 3) return $alertas3;
			else return null;
	...
	*/
	...
	/*
	// Busca todos os Alertas de Falta Ativos
	...
	// Busca todos os Alertas de Horário Ativos
