...
// Inclui as classes e a conex„o com o banco de dados.
include ("./gerenciaUsuarios.php");
include ("classes/Usuario.php");
include ("banco_mysql.php");

// Cria um objeto para os usu·rios
//$repositorioUser = new RepositorioUsuarios($mysqli);

// Cria um objeto para um novo usu·rio
$user = new Usuario();

// Chama a funÁ„o que busca os usu·rios e armazena em uma vari·vel
...
<body style="background-color:#F8F8FF;">
	...
	<br>
	<h3 align="center">Cadastro Novo Usu√°rio</h3>
	<br>

    <center>
        <br>
        <?php 
		// Quando o usu·rio clicar no bot„o cadastrar, executa esse 'if'
		if(isset($_POST["cadastrar"])){
            $erro = 0;
			
			// Faz o tratamento das vari·veis que s„o capturadas pelo formul·rio
            ...
            foreach($usuariosJaCadastrados as $usuarioAntigo){
				// Verifica se j· tem algum usu·rio com esse login
                if($login === $usuarioAntigo["login"]){
                    $erro = 1;
                    $msgErro .= "O login j· esta em uso.<br>";
                }
				// Verifica se j· tem algum usu·rio com esse email
                if($email === $usuarioAntigo["email"]){
                    $erro = 1;
                    $msgErro .= "O email j· esta em uso.<br>";
                }
            }
            ...
            if($erro != 0){
				...
				// Caso n„o tenha erro ele seta os dados que vieram do formul·rio
                $user->setEmail($email);
                $user->setLogin($login);
                //$user->setSenha($senha);
                $user->setAdm($adm);
