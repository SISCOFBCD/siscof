...
if(isset($_GET["id"])){
	...
	$alerta = buscaAHporID($AHid);
}

?>

	<h3 align="center">Reativar Alerta de Horário</h3>
	<br>
	...
	<form method="post" action="alertas/cadastraAlertaH.php" class="form-horizontal">
			...
			<div class="form-group">
				...
				<div class="input-group">
					...
					<input name="limiar_tempo" id="limiar_tempo" type="text" required="required" class="form-control" placeholder="minutos" value=<?php echo utf8_encode($alerta["limiar_tempo"]); ?> >
				</div>   
			</div>
			...
			<div class="form-group">
				<div class="input-group">
					<table><tr>
						<td><input type="radio" name="gender" value="chegada" <?php if($alerta["chegada"]>0){ ?> checked <?php } ?> > Chegada Tardia<br></td>
						<td><input type="radio" name="gender" value="saida" style="margin-left:10px;" <?php if($alerta["chegada"]==0) { ?> checked <?php } ?> > Saída Antecipada<br></td>
					</tr></table>
				</div>   
			</div>
